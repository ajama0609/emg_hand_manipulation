   ...
   % ch=1; 
    ...
    num_windows = length(filtered_windows);
num_channels = size(filtered_windows{1}, 2);

% Preallocate feature matrices
RMS_feat = zeros(num_windows, num_channels);
MAV_feat = zeros(num_windows, num_channels);
WL_feat = zeros(num_windows, num_channels);
ZC_feat = zeros(num_windows, num_channels);
SSC_feat = zeros(num_windows, num_channels);
VAR_feat = zeros(num_windows, num_channels);

threshold_zc = 1e-5;  % zero crossing threshold to avoid noise

for w = 1:num_windows
    window = filtered_windows{w};
    
    for ch = 1:num_channels
        x = window(:, ch);
        
        % RMS
        RMS_feat(w, ch) = sqrt(mean(x.^2));
        
        % MAV
        MAV_feat(w, ch) = mean(abs(x));
        
        % Waveform Length
        WL_feat(w, ch) = sum(abs(diff(x)));
        
        % Zero Crossing
        zc = 0;
        for i = 2:length(x)
            if ((x(i)*x(i-1) < 0) && (abs(x(i)-x(i-1)) >= threshold_zc))
                zc = zc + 1;
            end
        end
        ZC_feat(w, ch) = zc;
        ...
        for i = 2:length(x)-1
            diff1 = x(i) - x(i-1);
            diff2 = x(i) - x(i+1);
            if ((diff1*diff2) > 0) && (abs(diff1) >= threshold_zc) && (abs(diff2) >= threshold_zc)
                ssc = ssc + 1;
            end
        end
        SSC_feat(w, ch) = ssc;
...
% Concatenate features into one matrix
features = [RMS_feat, MAV_feat, WL_feat, ZC_feat, SSC_feat, VAR_feat];
